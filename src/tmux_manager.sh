...
stop_session() {
  ...
  if tmux has-session -t "$SESSION_NAME" 2>/dev/null; then
    ...
    tmux send-keys -t "$SESSION_NAME:0.1" "/exit" && sleep 0.1 && tmux send-keys -t "$SESSION_NAME:0.1" Enter 2>/dev/null || true

    # Send Ctrl+C and exit to developer pane
    tmux send-keys -t "$SESSION_NAME:0.2" C-c 2>/dev/null || true
    tmux send-keys -t "$SESSION_NAME:0.2" "/exit" && sleep 0.1 && tmux send-keys -t "$SESSION_NAME:0.2" Enter 2>/dev/null || true
